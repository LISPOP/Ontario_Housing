...
val_labels(on22$time_flag)<-c("Less than 1 hour"=0, "More than 1 hour"=1)
on22 %>% 
  ggplot(., aes(x=Duration__in_seconds_))+geom_histogram()+facet_wrap(~as_factor(time_flag), scales="free_x")
