...
names(census_data)
census_data %>% 
  rename_with(., ~paste0(., "_da"))->census_data
write_csv(census_data, file=here("Data/ontario_statscan_data_da.csv"))
#Get CSD level data
#Get CSD data
census_data_csd <- get_census(dataset='CA21',regions=list(PR="35"),
                          vectors=c("v_CA21_434","v_CA21_435","v_CA21_440","v_CA21_436","v_CA21_437","v_CA21_438","v_CA21_439","v_CA21_4290", "v_CA21_4309", "v_CA21_4317"),
                          labels="detailed", geo_format=NA, level='CSD')
#Get CSD data
names(census_data_csd)
census_data_csd %>%
...
census_data_csd %>%
  mutate(single_detached_houses_pct=single_detached_houses/total_occupied_private_dwellings,
         ...
         apartment_in_duplex_pct=apartment_in_duplex/total_occupied_private_dwellings,
         apartment_in_building_less_5_pct=apartment_in_building_less_5/total_occupied_private_dwellings,
         apartment_in_building_plus_5_pct=apartment_in_building_plus_5/total_occupied_private_dwellings)->census_data_csd
#Create population density
census_data_csd %>%
  mutate(pop_density=Population/`Area (sq km)`)->census_data_csd

census_data_csd %>% 
  rename_with(., ~paste0(., "_csd"))->census_data_csd
write_csv(census_data_csd, file=here("Data/ontario_statscan_data_csd.csv"))
