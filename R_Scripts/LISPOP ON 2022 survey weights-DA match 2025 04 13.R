...
data.0 <- read_sav("opes22_2022-09-24.sav")

var_names_labels <- as_tibble(colnames(data.0)) %>%
  rename(vname = value) %>%
  bind_cols(as_tibble(sjlabelled::get_label(data.0))) %>%
  rename(vlab = value)

xtabs(~ sjlabelled::as_character(Q38), addNA = TRUE, na.action = NULL, data = data.0)
xtabs(~ yob, addNA = TRUE, na.action = NULL, data = data.0)
xtabs(~ sjlabelled::as_character(Q39), addNA = TRUE, na.action = NULL, data = data.0)
xtabs(~ sjlabelled::as_character(Q40), addNA = TRUE, na.action = NULL, data = data.0)
...
## Read in PCCF
pccf.24 <- read_fwf(unz(paste0("C:/Users/timgr/OneDrive - Wilfrid Laurier University/Geocoding/",
                               "PCCF 2024.09/PCCF_FCCP_V2409_2021.zip"),
                        "PCCF_FCCP_V2409_2021.txt"),
...
data.2 <- data.1a %>%
  ...
  bind_rows(data.1e) %>%
  bind_rows(data.1i) %>%
  select(-psid, -non_partisan:-postal_code, -CSDNAME2021, -SLI, -Ret_Date:-FEDENAME) %>%
...
pumf.2021.0 <- read_csv(unz("C:/Users/timgr/OneDrive - Wilfrid Laurier University/Weighting/Canada/Census_2021_ind_PUMF.zip",
...
elec.2022.0 <- read_csv("Explorer_Party_09042025_144024.csv")
...
elec.2018.0 <- read_csv("Explorer_Party_09042025_144110.csv")
...
write_sav(data_out, "opes22_wtd_da_match_20250413.sav", compress = TRUE)
saveRDS(data_out, "opes22_wtd_da_match_20250413.rds")