...
table(on22$CSDtype)
  on22 %>% 
    ggplot(., aes(x=pop_density_csd))+geom_histogram()+facet_grid(~CSDtype)
table(on22$CSDname)
on22 %>% 
  select(cause_var_labels$variable, pop_density_csd) %>% 
  pivot_longer(., -pop_density_csd, names_to=c("variable")) %>% 
  left_join(., cause_var_labels) %>% 
  ggplot(., aes(x=log(pop_density_csd), y=value))+geom_point()+
  facet_wrap(~label, scales="free_y")+geom_smooth(method="lm")
#  ggplot(., aes(x=pop_density_csd))+geom_histogram()

  on22 %>% 
    select(solution_var_labels$variable, pop_density_csd) %>% 
    pivot_longer(., -pop_density_csd, names_to=c("variable")) %>% 
    left_join(., solution_var_labels) %>% 
    ggplot(., aes(x=log(pop_density_csd), y=value))+geom_point()+
    facet_wrap(~label, scales="free_y")+geom_smooth(method="lm")
...
names(on22)
on22 %>% 
select(cause_var_labels$variable, median_shelter_costs_owned_csd, median_shelter_costs_rented_csd) %>% 
  pivot_longer(., -c(median_shelter_costs_owned_csd, median_shelter_costs_rented_csd)) %>% 
  pivot_longer(., c(median_shelter_costs_owned_csd, median_shelter_costs_rented_csd), 
...
on22 %>% 
  select(solution_var_labels$variable, median_shelter_costs_owned_csd, median_shelter_costs_rented_csd) %>% 
  pivot_longer(., -c(median_shelter_costs_owned_csd, median_shelter_costs_rented_csd)) %>% 
  pivot_longer(., c(median_shelter_costs_owned_csd, median_shelter_costs_rented_csd), 
               names_to = c("Payment"), values_to=c("Cost")) %>% 
